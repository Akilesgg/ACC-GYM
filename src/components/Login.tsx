...
export default function Login({ language }: LoginProps) {
  ...
  const handleEmailAuth = async () => {
    ...
    try {
      ...
      if (err.code === 'auth/weak-password') msg = "La contraseña es muy corta";
      ...
      <main className="relative z-10 w-full max-w-md px-8 flex flex-col items-center">
        ...
        <div className="w-full space-y-6">
          <Button 
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-3 bg-primary text-background py-8 rounded-full font-headline font-bold text-lg active:scale-95 transition-all duration-200 shadow-[0_0_40px_rgba(243,255,202,0.2)] disabled:opacity-50"
          >
            {isLoading ? (
          ...
          <Button 
            ...
            disabled={isLoading}
            className="w-full py-8 rounded-full bg-surface-variant text-secondary font-headline font-bold tracking-wide hover:bg-surface active:scale-95 transition-all disabled:opacity-50"
          >
            {isLoading ? (
